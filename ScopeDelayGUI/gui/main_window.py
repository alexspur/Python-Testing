...
# #         # Default to 4 channels for DS7504
# #         self.rigol_num_channels = 4

# #         self.rigol2 = RigolScope("USB0::0x1AB1::0x0514::DS7A230800035::0::INSTR",
# #                                  num_channels=self.rigol_num_channels)  # Physical scope 1
# #         self.rigol3 = RigolScope("USB0::0x1AB1::0x0514::DS7A233300256::0::INSTR",
# #                                  num_channels=self.rigol_num_channels)  # Physical scope 2
# #         self.rigol1 = RigolScope("USB0::0x1AB1::0x0514::DS7A232900210::0::INSTR",
# #                                  num_channels=self.rigol_num_channels)  # Physical scope 3
...
# #         # Create scope plot window
# #         self.scope_window = ScopePlotWindow(parent=self, num_channels=self.rigol_num_channels)
...
# #         # Channel enable buttons (using clicked instead of toggled for checkable buttons)
# #         if hasattr(self.bnc_panel, "btn_en_a"):
...
# #     def set_rigol_channel_count(self, num_channels: int):
# #         """
# #         Change the number of channels to capture from all Rigol scopes.

# #         Args:
# #             num_channels: Number of channels (1-4)
# #         """
# #         self.rigol_num_channels = min(4, max(1, num_channels))

# #         # Update all scope instances
# #         for rigol in [self.rigol1, self.rigol2, self.rigol3]:
# #             if hasattr(rigol, 'set_num_channels'):
# #                 rigol.set_num_channels(self.rigol_num_channels)

# #         self.log(f"[RIGOL] Channel count set to {self.rigol_num_channels}")
...
# #     def on_bnc_apply(self):
# #         """Apply timing settings from panel to device"""
...
# #     def on_bnc_read(self):
# #         """Read timing settings from device to panel"""
# #         if not self.bnc_connected:
# #             self.error_popup("BNC575", "Not connected")
# #             return
...
# #     def on_bnc_arm(self):
# #         """Arm/disarm for external trigger"""
# #         if not self.bnc_connected:
# #             self.error_popup("BNC575", "Not connected")
# #             return
...
# #     def on_bnc_apply_trigger(self):
# #         """Apply trigger settings without arming"""
# #         if not self.bnc_connected:
# #             self.error_popup("BNC575", "Not connected")
# #             return
...
# #         """Apply system mode and period settings"""
# #         if not self.bnc_connected:
# #             self.error_popup("BNC575", "Not connected")
# #             return
...
# #             # Set burst count if in burst mode
...
# #         """Store configuration to device memory"""
# #         if not self.bnc_connected:
# #             self.error_popup("BNC575", "Not connected")
# #             return
...
# #         """Recall configuration from device memory"""
# #         if not self.bnc_connected:
# #             self.error_popup("BNC575", "Not connected")
# #             return
...
# #         self.single_worker = CaptureSingleWorker(rigol, name, use_multi_channel=True)

# #         # Connect both signals
# #         self.single_worker.finished.connect(lambda ch1, ch2, nm: self.on_single_capture_finished(ch1, ch2, plot_widget, nm))
# #         self.single_worker.finished_multi.connect(lambda data, nm: self.on_multi_channel_capture_finished(data, nm))
# #         self.single_worker.error.connect(lambda msg: self.on_single_capture_error(msg, name))
# #         self.single_worker.start()
...
# #     def on_multi_channel_capture_finished(self, data, name):
# #         """
# #         Handle multi-channel capture completion.

# #         Args:
# #             data: Dict mapping "CHANx" to (time, voltage) tuples
# #             name: Scope name (e.g., "Rigol #1")
# #         """
# #         # Determine which scope
# #         scope_id = 0
# #         if "1" in name:
# #             self.scope_window.update_r1(data=data)
# #             scope_id = 1
# #         elif "2" in name:
# #             self.scope_window.update_r2(data=data)
# #             scope_id = 2
# #         elif "3" in name:
# #             self.scope_window.update_r3(data=data)
# #             scope_id = 3

# #         # Log capture with channel count info
# #         num_points = sum(len(d[0]) for d in data.values())
# #         self.data_logger.log_scope_capture(scope_id, num_points, len(data))

# #         self.set_status("green", f"{name} captured ({len(data)} channels)")
# #         self.log(f"[{name}] capture complete - {len(data)} channels.")
...
# #         # -------------------------------------------
# #         # 5. CAPTURE waveforms (multi-channel)
# #         # -------------------------------------------
# #         self.set_status("yellow", "Capturing waveforms...")

# #         if self.rigol1_connected:
# #             data = self.rigol1.wait_and_capture()
# #             if data:
# #                 self.scope_window.update_r1(data=data)
# #                 self.data_logger.log_scope_capture(1, len(data), sum(len(d[0]) for d in data.values()))

# #         if self.rigol2_connected:
# #             data = self.rigol2.wait_and_capture()
# #             if data:
# #                 self.scope_window.update_r2(data=data)
# #                 self.data_logger.log_scope_capture(2, len(data), sum(len(d[0]) for d in data.values()))

# #         if self.rigol3_connected:
# #             data = self.rigol3.wait_and_capture()
# #             if data:
# #                 self.scope_window.update_r3(data=data)
# #                 self.data_logger.log_scope_capture(3, len(data), sum(len(d[0]) for d in data.values()))
...
#         self.start_single_capture(self.rigol3, self.scope_window.plot3, "Rigol #3")
#     def start_single_capture(self, rigol, plot_widget, name):
#         self.set_status("yellow", f"Waiting for {name} trigger...")
#         self.log(f"[{name}] Arming and waiting for trigger...")

#         # Arm the scope - put it in SINGLE mode waiting for trigger
#         rigol.arm()
        
#         # Store plot widget reference for the finished handler
#         self._current_capture_plot = plot_widget
#         self._current_capture_name = name
        
#         self.single_worker = CaptureSingleWorker(rigol, name, use_multi_channel=True)
#         self.single_worker.finished.connect(self._on_capture_finished_legacy)
#         self.single_worker.finished_multi.connect(self._on_capture_finished_multi)
#         self.single_worker.error.connect(lambda msg: self.on_single_capture_error(msg, name))
#         self.single_worker.start()

#     def _on_capture_finished_legacy(self, ch1, ch2, name):
#         """Handle legacy 2-channel capture completion"""
#         plot_widget = getattr(self, '_current_capture_plot', None)
#         if plot_widget:
#             self.on_single_capture_finished(ch1, ch2, plot_widget, name)

#     def _on_capture_finished_multi(self, data, name):
#         """Handle multi-channel capture completion"""
#         plot_widget = getattr(self, '_current_capture_plot', None)
#         if plot_widget:
#             self.on_multi_channel_capture_finished(data, name, plot_widget)
...
#     def on_multi_channel_capture_finished(self, data, name, plot_widget=None):
#         """
#         Handle multi-channel capture completion.
...
#             data: Dict mapping "CHANx" to (time, voltage) tuples
#             name: Scope name (e.g., "Rigol #1")
#             plot_widget: The plot widget to update (optional, used for scope identification)
#         """
#         # Determine which scope based on name or plot_widget
#         scope_id = 0
#         if plot_widget == self.scope_window.plot1 or "1" in name:
#             self.scope_window.update_r1(data=data)
#             scope_id = 1
#         elif plot_widget == self.scope_window.plot2 or "2" in name:
#             self.scope_window.update_r2(data=data)
#             scope_id = 2
#         elif plot_widget == self.scope_window.plot3 or "3" in name:
#             self.scope_window.update_r3(data=data)
#             scope_id = 3

#         # Log capture with channel count info
#         num_channels = len(data)
#         num_points = sum(len(d[0]) for d in data.values()) if data else 0
#         self.data_logger.log_scope_capture(scope_id, num_points, num_channels)

#         self.set_status("green", f"{name} captured ({num_channels} channels)")
#         self.log(f"[{name}] capture complete - {num_channels} channels.")
